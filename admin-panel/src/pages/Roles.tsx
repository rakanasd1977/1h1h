...
const ACTION_LABELS: Record<string, string> = {
  view: 'ad.act.view', create: 'ad.act.create', edit: 'ad.act.edit', delete: 'ad.act.delete', export: 'ad.act.export',
};
...
export default function Roles() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t('ad.roles.deleted'));
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      toast.success(t('ad.roles.roleAssigned'));
      setAssignModal(null);
      load();
    } catch (e: any) { toast.error(e.message); }
  };
  const handleUnassign = async () => {
    ...
    try {
      ...
      toast.success(t('ad.roles.roleUnassigned'));
  ...
  return (
    <div>
      <PageHead title={t('ad.roles.title')} subtitle={t('ad.roles.subtitle')} actions={<><div className="flex gap-sm">
          <div className="filters" style={{ gap: 4 }}>
            <button className={`tab ${tab === 'roles' ? 'tab--active' : ''}`} onClick={() => setTab('roles')}>{t('ad.roles.tabRoles')}</button>
            {can('users', 'view') && (
              <button className={`tab ${tab === 'users' ? 'tab--active' : ''}`} onClick={() => { setTab('users'); loadUsers(); }}>{t('ad.roles.tabAssign')}</button>
            )}</div>
          {tab === 'roles' && can('roles', 'create') && (
            <button className="btn btn-primary" onClick={openCreate}>➕ {t('ad.roles.newBtn')}</button>
          )}
        </div></>} />

      {tab === 'roles' && (
        roles.length === 0 ? (
          <div className="card"><div className="card-body" style={{ textAlign: 'center', padding: 40 }}>{t('ad.roles.noRoles')}</div></div>
        ) : (
                ...
                <div className="card" key={role.id}>
                  <div className="card-body">
                    <div className="flex-between" style={{ marginBottom: 8 }}>
                      ...
                      {role.is_system
                        ? <Badge status="system" map={{ system: { label: t('ad.roles.systemBadge'), cls: 'badge-teal' } }} />
                        : <span className="badge badge-gray">{t('ad.roles.customBadge')}</span>}
                    </div>
                    <div className="muted mb-2" style={{ fontSize: 13 }}>{role.description || '—'}</div>

                    <div className="flex gap-sm mb-2" style={{ fontSize: 12 }}>
                      <span className="badge badge-blue">{t('ad.roles.usersCount', { count: fmt(role.users_count || 0) })}</span>
                      {sum.full
                        ? <span className="badge badge-green">{t('ad.roles.fullAccess')}</span>
                        : <span className="badge badge-amber">{t('ad.roles.permsCount', { count: fmt(sum.count) })}</span>}
                    </div>
                        ...
                        {Object.entries(grouped).map(([res, actions]: [string, any]) => (
                          <div key={res} style={{ marginBottom: 6 }}>
                            ...
                            <div className="flex gap-xs flex-wrap" style={{ marginTop: 2 }}>
                              {[...actions].map((a) => (
                                <span key={a} className="badge badge-gray" style={{ fontSize: 11 }}>{ACTION_LABELS[a] ? t(ACTION_LABELS[a]) : a}</span>
                              ))}
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="flex gap-sm mt-3">
                      {can('roles', 'edit') && role.name !== 'super_admin' && (
                        <button className="btn btn-outline btn-sm" onClick={() => openEdit(role)}>{t('ad.roles.editPerms')}</button>
                      )}
                      {!role.is_system && can('roles', 'delete') && (
                        <button className="btn btn-danger btn-sm" onClick={() => setDeleteConfirm(role)}>{t('ad.common.delete')}</button>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {tab === 'users' && (
        users.length === 0 ? (
          <div className="card"><div className="card-body" style={{ textAlign: 'center', padding: 40 }}>{t('ad.roles.noUsers')}</div></div>
        ) : (
          <div className="card">
            <div className="table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>{t('ad.roles.thAdmin')}</th>
                    <th>{t('ad.roles.thEmail')}</th>
                    <th>{t('ad.roles.thStatus')}</th>
                    <th>{t('ad.roles.thRoles')}</th>
                    <th>{t('ad.roles.thActions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td><span className={`badge ${user.is_active ? 'badge-green' : 'badge-red'}`}>{user.is_active ? t('ad.roles.active') : t('ad.roles.suspended')}</span></td>
                      <td>
                        <div className="flex gap-xs flex-wrap">
                          ...
                          {(user.assigned_roles || []).length === 0 && <span className="muted">{t('ad.roles.none')}</span>}
                        </div>
                      </td>
                      <td>
                        {can('users', 'edit') && (
                          <button className="btn btn-outline btn-sm" onClick={() => openAssign(user)}>{t('ad.roles.assignRole')}</button>
                        )}
                      </td>
                    </tr>
                  ))}
      ...
      <Confirm open={!!deleteConfirm} title={t('ad.roles.deleteTitle')} message={t('ad.roles.deleteMsg', { name: deleteConfirm?.name_ar })} onConfirm={handleDelete} onCancel={() => setDeleteConfirm(null)} danger />

      <Confirm open={!!deleteAssign} title={t('ad.roles.unassignTitle')} message={t('ad.roles.unassignMsg')} onConfirm={handleUnassign} onCancel={() => setDeleteAssign(null)} danger />

      <Modal open={showModal} title={editingRole ? t('ad.roles.editModalTitle', { name: editingRole.name_ar }) : t('ad.roles.createModalTitle')} onClose={() => setShowModal(false)} size="lg">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-2 mb-3">
            {!editingRole && (
              <Field label={t('ad.roles.fieldNameEn')} required>
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="content_manager" />
              </Field>
            )}
            <Field label={t('ad.roles.fieldNameAr')} required>
              <input value={form.name_ar} onChange={(e) => setForm({ ...form, name_ar: e.target.value })} placeholder={t('ad.roles.nameArPlaceholder')} />
            </Field>
          </div>
          <Field label={t('ad.roles.fieldDescription')} full>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} placeholder={t('ad.roles.descPlaceholder')} />
          </Field>

          {editingRole && editingRole.is_system && !isSuperAdmin(editingRole) && (
            <div className="alert-info mb-3" style={{ fontSize: 13 }}>⚠️ {t('ad.roles.systemNote')}</div>
          )}
          {isSuperAdmin(editingRole) && (
            <div className="alert-success mb-3" style={{ fontSize: 13 }}>✅ {t('ad.roles.superAdminNote')}</div>
          )}

          <div className="card mb-3">
            <div className="card-header flex-between">
              <h3>{t('ad.roles.matrixTitle')}</h3>
              {matrixEditable(editingRole) && (
                <div className="flex gap-xs">
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => selectAll(true)}>{t('ad.roles.selectAll')}</button>
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => selectAll(false)}>{t('ad.roles.clearAll')}</button>
                </div>
              )}
            </div>
            <div className="card-body">
              {!matrixEditable(editingRole) ? (
                <div className="alert-success" style={{ fontSize: 13 }}>{t('ad.roles.fullPermsNote')}</div>
              ) : (
                    ...
                    <div key={r.key} style={{ padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                      <div className="flex-between mb-1">
                        ...
                        <label className="flex items-center gap-xs" style={{ fontSize: 12, cursor: 'pointer' }}>
                          ...
                          />
                          <span className="muted">{t('ad.roles.all')}</span>
                        </label>
                      </div>
                      <div className="flex gap-sm flex-wrap">
                        {r.actions.map((a: string) => (
                          <label key={a} className="flex items-center gap-xs" style={{ fontSize: 13, cursor: 'pointer' }}>
                            ...
                            />
                            <span>{ACTION_LABELS[a] ? t(ACTION_LABELS[a]) : a}</span>
                          </label>
                        ))}
                      </div>
                    </div>
          ...
          <div className="flex gap-sm justify-end">
            <button type="button" className="btn btn-outline" onClick={() => setShowModal(false)}>{t('ad.common.cancel')}</button>
            {(editingRole ? can('roles', 'edit') : can('roles', 'create')) && (
              <button type="submit" className="btn btn-primary">{editingRole ? t('ad.roles.saveChanges') : t('ad.roles.createRole')}</button>
            )}
          </div>
        </form>
      </Modal>

      <Modal open={!!assignModal} title={t('ad.roles.assignModalTitle', { name: assignModal?.name_ar })} onClose={() => setAssignModal(null)} size="sm">
        <div className="mb-3">
          <label className="block mb-1">{t('ad.roles.chooseRole')}</label>
          <select value={newRoleId} onChange={(e) => setNewRoleId(e.target.value)} className="input" style={{ width: '100%' }}>
            <option value="">{t('ad.roles.chooseRoleOption')}</option>
        ...
        <div className="flex gap-sm justify-end">
          <button className="btn btn-outline" onClick={() => setAssignModal(null)}>{t('ad.common.cancel')}</button>
          <button className="btn btn-primary" onClick={handleAssign} disabled={!newRoleId}>{t('ad.roles.assignBtn')}</button>
        </div>
      </Modal>
    </div>
  );
