    ...
    <div className="gov-list">
      {loading ? (
        ...
        <div className="empty">
          ...
          <div className="empty__title">{t('gov.failedTitle')}</div>
          <div className="empty__sub">{t('loyalty.errorSub')}</div>
          <button className="btn btn--primary" type="button" onClick={() => window.location.reload()}>{t('loyalty.retry')}</button>
        </div>
      ) : (
        governorates.map((g) => (
          <button
            ...
            onClick={() => onSelect(g)}
          >
            <span className="gov-item__name">{g.name || g.name_ar}</span>
            {g.providers_count != null && g.providers_count > 0 && <span className="gov-item__count">{t('gov.providers', { count: g.providers_count })}</span>}
...
function GovGate({ governorates, onSelect, loading }: GovGateProps) {
  ...
  return (
    <div className="gov-gate">
      ...
      <div className="gov-gate__title">{t('gov.chooseTitle')}</div>
      <div className="gov-gate__sub">{t('gov.chooseSub')}</div>
  ...
  return (
    <div className="sheet-overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sheet__title">{t('gov.currentTitle')}</div>
      ...
      {loading || resolving ? (
        <div className="page" aria-busy="true">
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--muted)' }}>
            <CenterSpinner />
            <div style={{ marginTop: 12, fontSize: 13 }}>{t('gov.resolving')}</div>
          </div>
        </div>
      ) : !gov ? (
