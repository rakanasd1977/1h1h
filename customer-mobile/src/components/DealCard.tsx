...
export const DealCard = memo(function DealCard({ deal = {}, stock = 0 }: any) {
  ...
  const soldLabel = deal.kind === 'products' ? t('deal.soldProducts') : t('deal.soldOrders');
  return (
    <button className="deal-card" type="button" onClick={() => navigate(to)}>
      ...
      <div className="deal-card__body">
        ...
        <div className="deal-card__provrow">
          {deal.provider_verified && <span className="deal-card__verified">{t('deal.verified')}</span>}
          <span className="deal-card__prov">{deal.provider_name}</span>
        </div>
        {typeof stock === 'number' && (
          <div className="deal-card__stock">
            <span className="deal-card__stock-txt">{t('deal.left', { stock })}</span>
