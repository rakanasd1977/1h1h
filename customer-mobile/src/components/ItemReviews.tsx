...
export function ItemReviews({ kind, itemId, onToast }: { kind: string; itemId: number | string; onToast?: (msg: string) => void }) {
  ...
  const submit = async () => {
    if (myRating < 1) {
      if (onToast) onToast(t('reviews.rateError'));
      return;
    }
    setBusy(true);
    try {
      await customerApi.rateItem(kind, itemId, myRating, myComment.trim() || undefined);
      if (onToast) onToast(t('reviews.thanks'));
      await refresh();
    } catch (e: any) {
    ...
    <div>
      {user ? (
        <div className="card" style={{ padding: 14, marginBottom: 12 }}>
          <div className="muted" style={{ marginBottom: 6 }}>{myRating ? t('reviews.editYourRating') : t('reviews.rateThisItem')}</div>
      ...
      {reviews.length ? (
        ...
        <div className="muted" style={{ padding: '10px 0' }}>{t('reviews.empty')}</div>
      )}
    </div>
  );
}
