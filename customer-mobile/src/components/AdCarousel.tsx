...
export function AdCarousel({ ads, interval = 4500, variant }: { ads: ApiRecord[]; interval?: number; variant?: string }) {
  ...
  return (
    ...
    >
      ...
      <button className="ad-carousel__arrow ad-carousel__arrow--next" onClick={() => go(index + 1)} aria-label={t('common.next')}>‹</button>
      <button className="ad-carousel__arrow ad-carousel__arrow--prev" onClick={() => go(index - 1)} aria-label={t('common.prev')}>›</button>
      <div className="ad-carousel__dots">
        {ads.map((a, i) => (
          <button key={a.id} className={`dot${i === index ? ' active' : ''}`} onClick={() => go(i)} aria-label={t('ad.dot', { n: i + 1 })} />
        ))}
      </div>
    </div>
  );
