...
export default function Flights() {
  ...
  const book = (f: any) => {
    ...
    addItem({
      ...
      quantity: passengers,
      unit: t('item.unit.flight'),
      booking: { type: 'flights', travel_date: travelDate, passengers },
    });
    setToast(t('flights.added'));
  };

  const route = (f: any) => `${f.origin_ar || f.origin} → ${f.destination_ar || f.destination}`;

  return (
    <div>
      <PageHeader title={t('flights.title')} />
      <div className="page page--no-nav">
        ...
        {loading ? (
          ...
          flights.map((f) => (
            <div className="card" key={f.id} style={{ marginBottom: 10, padding: 12, position: 'relative' }}>
              ...
              <div className="row row--between">
                <div>
                  <div style={{ fontWeight: 800, fontSize: 15 }} onClick={() => navigate(`/item/${id}/flights/${f.id}`)}>{route(f)}</div>
                  <div className="muted" style={{ marginTop: 2 }}>
                    {f.airline} {f.flight_number ? `· ${t('flights.flightNumber', { number: f.flight_number })}` : ''}
                  </div>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div className="muted">{t('flights.perPassenger')}</div>
                </div>
              </div>
              <div className="row row--between" style={{ marginTop: 10 }}>
                <span className="muted">{t('flights.seatsAvailable', { count: f.seats })}</span>
                <button className="btn btn--primary btn--sm" onClick={() => book(f)} type="button">
                  {t('flights.bookSeats', { count: passengers })}
                </button>
              </div>
              <button
                ...
                type="button"
              >
                ⭐ {t('reviews.label')}{Number(f.rating_count) > 0 ? ` (${f.rating_count})` : ''}
              </button>
              {openRevId === f.id && (
          ...
          <EmptyState icon="✈️" title={t('flights.emptyTitle')} sub={t('flights.emptySub')} />
        )}
      </div>
