...
const SORTS = [
  { v: '', label: 'search.sortRated' },
  { v: 'sold', label: 'search.sortSold' },
  { v: 'recent', label: 'search.sortRecent' },
  { v: 'name', label: 'search.sortName' },
];
      ...
      {loading ? (
        ...
        <>
          <div className="row row--between" style={{ marginBottom: 8 }}>
            <span className="muted">{total > 0 ? t('gov.providers', { count: total }) : t('search.label')}</span>
            <div className="seg" style={{ padding: 0 }}>
              {SORTS.map((s) => (
                <button
                  key={s.v}
          ...
          {providers.length ? (
            ...
            <EmptyState
              ...
              sub={q ? t('search.noResultsSub', { q, gov: governorate ? governorate.name || governorate.name_ar : '' }) : t('search.noResultsAlt')}
            />
          )}
        </>
      )}
