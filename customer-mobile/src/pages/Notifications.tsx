...
export default function Notifications() {
  ...
  return (
    <div className="page">
      <PageHeader
        title={t('notif.title')}
        right={unread > 0 ? <button className="btn btn--ghost btn--sm" onClick={readAll} type="button">{t('notif.readAll')}</button> : null}
      />

      {!items ? (
        <div className="centerpad"><Spinner /></div>
      ) : items.length === 0 ? (
        <EmptyState icon="🔔" title={t('notif.emptyTitle')} sub={t('notif.emptySub')} />
      ) : (
