    ...
    return (
      <div className="page">
        <EmptyState
          ...
          sub={t('cart.emptySub')}
          action={<button className="btn btn--primary" onClick={() => navigate('/search')} type="button">{t('cart.browse')}</button>}
        />
      </div>
    );
  }
  ...
  return (
    <div className="page">
      <div className="row row--between" style={{ marginBottom: 10 }}>
        <div className="section-title" style={{ margin: 0 }}>{t('cart.title', { count: totalCount })}</div>
      </div>
          ...
          {g.items.map((it) => (
            <div key={`${it.kind}:${it.item_id}`} className="order-item">
              <div className="row" style={{ gap: 10 }}>
                ...
                <div className="grow">
                  ...
                  <div className="order-item__meta">
                    {formatPrice(it.unit_price)}/{it.unit || t('cart.unit')}
              ...
              <div className="row row--between" style={{ marginTop: 8 }}>
                ...
                <span className="muted">{t('cart.perProvider')}</span>
              </div>
            </div>
          ))}
        </div>
