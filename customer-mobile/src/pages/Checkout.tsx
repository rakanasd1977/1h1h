...
export default function Checkout() {
  ...
  const applyCoupon = async (rawCode?: string) => {
    ...
    if (!code) {
      setToast(t('checkout.enterCoupon'));
      return;
    }
    try {
      const rows = [];
      ...
      for (const g of groups) {
        ...
        if (!r.valid) {
          setToast(t('checkout.couponInvalid', { message: r.message }));
          setCoupon(null);
          return;
        }
        appliedCode = r.code;
      ...
      setToast(t('checkout.couponApplied'));
    } catch (e: any) {
      setToast(e.message || t('checkout.couponError'));
    }
  };
    ...
    return (
      <div>
        ...
        <div className="empty">
          ...
          <div className="empty__title">{t('checkout.loginRequired')}</div>
          <button className="btn btn--primary" onClick={() => navigate('/login?next=checkout')} type="button">{t('checkout.login')}</button>
        </div>
      </div>
    );
  }
  ...
  const submit = async () => {
    if (!name.trim() || !phone.trim()) {
      setToast(t('checkout.requiredFields'));
      return;
    }
    setSubmitting(true);
    setToast('');
    try {
      ...
      setToast(e.message || t('checkout.failed'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div>
      ...
      <div className="page page--no-nav">
        {addresses && addresses.length > 0 && (
          <div className="card" style={{ padding: 12, marginBottom: 12 }}>
            <div className="row row--between" style={{ marginBottom: 8 }}>
              <span style={{ fontWeight: 800, fontSize: 14 }}>{t('checkout.addresses')}</span>
              <Link to="/addresses" style={{ color: 'var(--brand)', fontSize: 12, fontWeight: 700 }}>{t('checkout.manage')}</Link>
            </div>
        ...
        <div className="card" style={{ padding: 14, marginBottom: 12 }}>
          ...
          <div className="field">
            <label>{t('checkout.addressOptional')}</label>
            <input className="input" value={address} onChange={(e) => setAddress(e.target.value)} />
          </div>
          <div className="field" style={{ marginBottom: 0 }}>
            <label>{t('checkout.notesOptional')}</label>
        ...
        <div className="card" style={{ padding: 14, marginBottom: 12 }}>
          ...
          {coupon && (
            <div className="muted" style={{ marginTop: 8, color: '#00a650', fontWeight: 700 }}>
              {t('checkout.couponAppliedLine', { code: coupon.code, amount: formatPrice(discount) })}
            </div>
          )}
        </div>
        ...
        {loyalty && balance >= minRedeem && (
          <div className="card" style={{ padding: 14, marginBottom: 12 }}>
            <div className="row" style={{ gap: 8, marginBottom: 8 }}>
              <span style={{ fontWeight: 800 }}>{t('checkout.loyaltyPoints')}</span>
              <span className="muted" style={{ fontSize: 12 }}>{t('checkout.balanceLine', { count: balance, value: formatPrice(balance * pointValue) })}</span>
            </div>
            <div className="row" style={{ gap: 8 }}>
              <input
                className="input"
                ...
                min={0}
                max={balance}
                placeholder={t('checkout.redeemPlaceholder')}
            ...
            {redeemPoints > 0 && (
              <div className="muted" style={{ marginTop: 8, color: '#00a650', fontWeight: 700 }}>
                {t('checkout.redeemLine', { count: redeemPoints, amount: formatPrice(redeemDiscount) })}
              </div>
            )}
          </div>
        )}
        ...
        {groups.map((g) => (
          <div className="card" key={g.provider_id} style={{ marginBottom: 12, padding: 14 }}>
            <div className="row row--between">
              ...
              {done.includes(g.provider_id) ? <span style={{ color: '#00a650', fontWeight: 700 }}>{t('checkout.done')}</span> : null}
            </div>
            {g.items.map((it) => (
            ...
            <div className="summary-row">
              <span>{t('checkout.providerTotal')}</span>
        ...
        <div className="card" style={{ padding: 14 }}>
          ...
          {redeemPoints > 0 && (
            <div className="summary-row" style={{ color: '#00a650' }}>
              <span>{t('checkout.pointsDiscount', { count: redeemPoints })}</span>
          ...
          <button className="btn btn--primary btn--lg" style={{ marginTop: 10 }} onClick={submit} disabled={submitting} type="button">
            {submitting ? <Spinner /> : t('checkout.confirmOrder')}
          </button>
        </div>
      </div>
