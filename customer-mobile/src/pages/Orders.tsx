...
const TABS = [{ v: '', label: 'orders.all' }, ...STATUS_OPTIONS.map((s) => ({ v: s, label: ORDER_STATUS[s].label }))];
...
export default function Orders() {
  ...
  return (
    <div className="page">
      <div className="section-title" style={{ marginTop: 6 }}>{t('orders.title')}</div>
      <div className="chips">
        {TABS.map((tab) => (
          <button key={tab.v} className={`chip${status === tab.v ? ' chip--active' : ''}`} onClick={() => setStatus(tab.v)} type="button">
            {t(tab.label)}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {hasMore && (
            <button className="btn btn--outline btn--block" onClick={loadMore} disabled={loadingMore} type="button" style={{ marginTop: 6 }}>
              {loadingMore ? t('orders.loadingMore') : t('orders.showMore')}
            </button>
          )}
        </>
      ) : (
        <EmptyState
          ...
          action={<button className="btn btn--primary" onClick={() => navigate('/search')} type="button">{t('orders.browse')}</button>}
        />
      )}
    </div>
  );
