  ...
  const sectionNodes: Record<string, any> = {
    ...
    coupons: coupons.length > 0 ? (
      <section key="coupons" className="home-block">
        <SectionHead icon="🎟️" title={t('home.coupons')} moreTo="/coupons" />
        <div className="hscroll">
    ...
    featured_providers: (
      <section key="featured_providers" className="home-block">
        <SectionHead icon="🏆" title={t('home.featuredProviders')} moreTo="/search" />
    ...
    recently_viewed: recent.length > 0 ? (
      <section key="recently_viewed" className="home-block">
        <SectionHead icon="🕘" title={t('home.recentlyViewed')} moreTo="/search" />
        <div className="hscroll">
          {recent.map((r) => (
    ...
    picks: picks.length > 0 ? (
      <section key="picks" className="home-block">
        <SectionHead icon="🔥" title={t('home.topPicks')} />
        <div className="hscroll">
    ...
    <div className="page">
      ...
      {loadError && (
        <div className="conn-banner" role="alert">
          ...
          <button className="conn-banner__btn" type="button" onClick={reload}>{t('common.retry')}</button>
        </div>
      )}
      ...
      {showTop && (
        <button className="backtop" type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label={t('home.backToTop')}>
          ↑
        </button>
      )}
