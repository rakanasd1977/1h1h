...
export default function Register() {
  ...
  const submit = async (e: any) => {
    ...
    try {
      ...
      navigate(`/verify?token=${encodeURIComponent(res.verification_token)}&email=${encodeURIComponent(form.email)}${next ? `&next=${next}` : ''}`);
    } catch (err: any) {
      setToast(err.message || t('auth.registerFailed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <div style={{ background: 'linear-gradient(120deg, var(--brand), var(--brand2))', padding: '24px 20px 48px', color: '#fff' }}>
        <div style={{ fontSize: 22, fontWeight: 800 }}>{t('auth.registerTitle')}</div>
      </div>
