...
export default function Provider() {
    ...
    publicApi
      ...
      .then((p) => {
        ...
        trackRecent({
          ...
          image: p.logo || null,
          sub: `${p.governorate_name || p.governorate_name_ar || ''} · ${p.service_name || p.service_name_ar || ''}`,
        });
      })
  ...
  const goToGov = () => {
    if (foreignGov) {
      ...
      const fakeGov: Governorate = { id: 0, code: p.governorate_code, name_ar: p.governorate_name_ar, name: p.governorate_name_ar };
      select(fakeGov);
    }
  };
  const sections = (
    [
      p.service_slug === 'stores' && { to: 'catalog', icon: '🛍️', label: 'provider.section.products', count: p.catalog_counts?.products },
      p.service_slug === 'restaurants' && { to: 'menu', icon: '🍽️', label: 'provider.section.menu', count: p.catalog_counts?.menu_items },
      p.service_slug === 'hotels' && { to: 'rooms', icon: '🛏️', label: 'provider.section.rooms', count: p.catalog_counts?.rooms },
      p.service_slug === 'flights' && { to: 'flights', icon: '✈️', label: 'provider.section.flights', count: p.catalog_counts?.flights },
      p.service_slug === 'travel_offices' && { to: 'packages', icon: '🧳', label: 'provider.section.packages', count: p.catalog_counts?.packages },
    ].filter(Boolean) as Array<{ to: string; icon: string; label: string; count?: number | string }>
  );

  const pname = p.name || p.name_ar;
  const govName = p.governorate_name || p.governorate_name_ar;
  const serviceName = p.service_name || p.service_name_ar;
  const following = isFollowing(p.id);

  return (
    <div>
      <PageHeader title={pname} />
      <div style={{ padding: 14 }}>
        {isForeign && (
          <div className="gov-banner">
            <span>{t('provider.foreign', { g: govName, current: governorate.name || governorate.name_ar })}</span>
        ...
        <div className="card" style={{ padding: 14 }}>
          <div className="row" style={{ gap: 12 }}>
            <div className="provider-card__logo" style={{ fontSize: 34, overflow: 'hidden' }}>
              {p.logo ? (
                <img src={p.logo} alt={pname} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                v.icon
              )}
            </div>
            <div className="grow">
              <div style={{ fontWeight: 800, fontSize: 17 }}>{pname}</div>
            ...
            <a
              ...
              aria-label={t('provider.shareWhats')}
            >
              💬
            </a>
            <button className="iconbtn share-icon" onClick={() => setShowShare(true)} type="button" aria-label={t('provider.share')}>
              🔗
            </button>
          </div>
          <button
            type="button"
            className={`followbtn${following ? ' followbtn--on' : ''}`}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              if (!user) {
              ...
              toggle(p.id).then((ok) => {
                if (ok) setToast(following ? t('provider.unfollowedToast') : t('provider.followedToast'));
                else setToast(t('provider.followError'));
              });
            }}
          >
            {following ? t('provider.following') : t('provider.follow')}
            {typeof p.followers_count === 'number' && p.followers_count > 0 && (
              <span className="followbtn__count">{t('provider.followers', { count: p.followers_count })}</span>
            )}
          </button>
          {p.is_verified && (
            <div className="chip chip--active" style={{ marginTop: 10, display: 'inline-block' }}>{t('provider.verified')}</div>
          )}
          {p.description && <p className="detail-desc">{p.description}</p>}
          {p.address && <div className="muted" style={{ marginTop: 6 }}>📍 {p.address}</div>}
        </div>

        <div className="section-title">{t('provider.catalog')}</div>
        {sections.map((s) => (
          <button
            key={s.to}
          ...
          >
            ...
            <span className="grow" style={{ fontWeight: 700, fontSize: 15 }}>{t(s.label)}</span>
        ...
        <div className="section-title">⭐ {t('reviews.label')}</div>
        {reviews && (
          <div className="card" style={{ padding: 14 }}>
                  ...
                  <div key={r.id} className="review">
                    ...
                    {r.reply && (
                      <div className="review__reply">
                        <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 2 }}>{t('provider.replyTitle')}</div>
                        {r.reply}
                      </div>
                    )}
                  </div>
      ...
      <ShareSheet
        ...
        onCopied={() => setToast(t('provider.copiedToast'))}
      />
