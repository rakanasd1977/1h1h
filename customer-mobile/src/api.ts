...
export const publicApi = {
  ...
  content: () => request('GET', '/public/content'),
};
...
export const authApi = {
  ...
  resendVerification: (payload: any) => request('POST', '/auth/resend-verification', payload),
  ...
  changeEmail: (email: string, current_password: string) => request('POST', '/auth/change-email', { email: email.trim().toLowerCase(), current_password }),
  forgotPassword: (email: string) => request('POST', '/auth/forgot-password', { email }),
  confirmResetPassword: (token: string, new_password: string) => request('POST', '/auth/forgot-password/confirm', { token, new_password }),
};

export const uploadApi = {
  avatar: (data: string) => request('POST', '/upload', { data }),
};
...
export const customerApi = {
  ...
  cancelOrder: (id: number | string, reason?: string) => request('PUT', `/orders/${id}/status`, { status: 'cancelled', reason }),
  ...
  myDisputes: (params: Record<string, any> = {}, raw = false) => {
    const qs = new URLSearchParams();
    for (const [k, v] of Object.entries(params || {})) if (v !== undefined && v !== null && v !== '') qs.set(k, String(v));
    if (!qs.has('limit')) qs.set('limit', '20');
    return request('GET', '/disputes' + (qs.toString() ? `?${qs.toString()}` : ''), undefined, { raw });
  },
  openDispute: (orderId: number | string, reason: string) => request('POST', `/orders/${orderId}/disputes`, { reason }),
  invoice: (orderId: number | string) => request('GET', `/orders/${orderId}/invoice`),
};