...
async function login() {
  ...
  if (!json.data || !json.data.user) throw new Error('تعذر تسجيل الدخول: ' + JSON.stringify(json));
  const setCookie = typeof r.headers.getSetCookie === 'function' ? r.headers.getSetCookie() : [r.headers.get('set-cookie')];
  const cookies = parseCookies(setCookie);
  return { session: cookies['rafidain_session'] || '', csrf: cookies['rafidain_csrf'] || '' };
}
  ...
  async function worker() {
    while (Date.now() < deadline) {
      ...
      try {
        const headers = {};
        if (auth.session) {
          headers['Cookie'] = auth.csrf ? `rafidain_session=${auth.session}; rafidain_csrf=${auth.csrf}` : `rafidain_session=${auth.session}`;
        }
        if (spec.method === 'POST' && auth.csrf) headers['X-CSRF-Token'] = auth.csrf;
