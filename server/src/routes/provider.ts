...
const verificationPutHandler = (req, res, next) => {
  provider.submitVerification(req.provider, req.user, req.body)
    .then((r) => ok(res, r))
    .catch((e: any) => next(e));
};

// GET /api/provider/documents/:field — يعرض المزود وثائقه هو فقط (عبر مسار مصادق)
const DOC_FIELDS = { national_id: 'national_id_image', residency: 'residency_doc_image' };
const providerDocumentsHandler = (req, res, next) => {
  try {
    const col = DOC_FIELDS[req.params.field];
    if (!col) throw new ApiError(400, 'حقل مستند غير معروف');
    const doc = req.provider[col];
    if (!doc) throw new ApiError(404, 'لا يوجد مستند مرفوع');
    if (!serveDocRef(res, doc)) throw new ApiError(404, 'الملف غير موجود');
  } catch (e: any) { next(e); }
};
...
router.get('/documents/:field', providerDocumentsHandler);
