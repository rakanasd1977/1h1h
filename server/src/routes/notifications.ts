...
const { getPreferences, setPreferences } = require('../services/notification-preferences');
...
const getPrefs = (req, res, next) => {
  try { ok(res, getPreferences(req.user.id)); } catch (e: any) { next(e); }
};

const setPrefs = (req, res, next) => {
  try { ok(res, setPreferences(req.user.id, req.body)); } catch (e: any) { next(e); }
};
...
router.put('/preferences', setPrefs);
