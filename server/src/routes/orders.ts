...
const { openDispute } = require('../services/disputes');
const { renderInvoice } = require('../services/invoices');
...
// نزاع على طلب (زبون/مزوّد)
router.post('/:id/disputes', (req, res, next) => {
  try {
    const party = ['provider', 'agent'].includes(req.user.role) ? req.user.role : 'customer';
    return created(res, openDispute(req.user, req.params.id, req.body, party));
  } catch (e: any) { next(e); }
});

// فاتورة طلب (JSON + سطور) — بنطاق الصلاحية
router.get('/:id/invoice', (req, res, next) => {
  try { return ok(res, renderInvoice(req.user, req.params.id)); } catch (e: any) { next(e); }
});
