...
function authenticate(req, res, next) {
  ...
  // معطَّل في التطوير عبر config.fingerprint.enabled لئلا يعيق لوحات التطوير.
  if (config.fingerprint.enabled && payload.jti && PRIVILEGED_ROLES.includes(user.role) && !String(payload.jti).startsWith('2fa-')) {
