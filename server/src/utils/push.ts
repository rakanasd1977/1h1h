...
const { getPreferences, categoryForType } = require('../services/notification-preferences');
...
if (vapidKeys) {
  webpush.setVapidDetails(
    process.env.VAPID_SUBJECT || 'mailto:support@rafidain.iq',
    vapidKeys.publicKey,
...
function createInAppNotification(userId, { type = 'order', title, body, url, icon }) {
  try {
    run(
      'INSERT INTO notifications (user_id, type, title, body, url, icon) VALUES (?,?,?,?,?,?)',
      [userId, type, String(title || '').slice(0, 300), String(body || '').slice(0, 600), String(url || '').slice(0, 300), String(icon || '').slice(0, 300)]
    );
...
// إشعار كامل: يحفظ داخلياً ثم يرسل Web Push لكل الأجهزة، وفق تفضيلات المستخدم.
// التفضيل الافتراضي "كل شيء مفعّل"؛ يُتجاهل الإشعار فقط إذا أوقف المستخدم القناة أو الفئة.
function notifyUser(userId, payload) {
  try {
    const prefs = getPreferences(userId);
    const category = categoryForType(payload && payload.type);
    const allowedCategory = !prefs.categories || prefs.categories[category] !== false;
    if (!allowedCategory) return Promise.resolve({ sent: 0, removed: 0, skipped: true });

    if (prefs.in_app) createInAppNotification(userId, payload);
    if (prefs.push) return sendPush(userId, payload);
    return Promise.resolve({ sent: 0, removed: 0 });
  } catch (e: any) {
    console.error('[notify] تعذر التحقق من التفضيلات، سيعتبر الإشعار مسموحاً:', e.message);
...
function notifyRole(role, payload) {
  ...
  return Promise.all(users.map((u) => notifyUser(u.id, payload))).then((results) =>
    results.reduce((acc, r) => ({ sent: acc.sent + r.sent, removed: acc.removed + r.removed }), { sent: 0, removed: 0 })
  );
}
...
function notifyProviderFollowers(providerId, payload) {
  const rows = all(
    `SELECT f.customer_id FROM provider_follows f
     JOIN users u ON u.id = f.customer_id
     WHERE f.provider_id = ? AND u.is_active = 1`,
    [providerId]
  );
  return Promise.all(rows.map((r) => notifyUser(r.customer_id, payload))).then((results) =>
    results.reduce((acc, r) => ({ sent: acc.sent + r.sent, removed: acc.removed + r.removed }), { sent: 0, removed: 0 })
  );
}
...
function sendPush(userId, { title, body, url, icon }) {
  if (!vapidKeys) return Promise.resolve({ sent: 0, removed: 0 });
  ...
  return Promise.all(subs.map((s) => {
    let subscription;
    try { subscription = { endpoint: s.endpoint, keys: JSON.parse(s.keys_json) }; } catch (e: any) { return Promise.resolve(null); }
    return webpush.sendNotification(subscription, payload)
      .then(() => null)
      .catch((err) => {
        const code = err && (err.statusCode || (err.response && err.response.statusCode));
        if (code === 404 || code === 410) {
          try { require('../db').run('DELETE FROM push_subscriptions WHERE id = ?', [s.id]); } catch (e: any) { /* تجاهل */ }
          return { removed: 1 };
        }
