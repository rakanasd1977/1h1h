...
function getTemplate(entity) {
  ...
  if (!tmpl) throw new Error('الجهة غير مدعومة. المدعومة: agents, providers, products, coupons');
  return tmpl;
}
...
function validateRows(entity, rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    switch (entity) {
      case 'agents':
        if (!row.name_ar?.trim()) errors.push('name_ar مطلوب');
        if (!row.email?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email)) errors.push('email غير صالح');
        if (!row.phone?.trim()) errors.push('phone مطلوب');
        if (!row.password?.trim()) errors.push('password مطلوب');
        if (!row.governorate_code?.trim()) errors.push('governorate_code مطلوب');
        break;
      case 'providers':
        if (!row.name_ar?.trim()) errors.push('name_ar مطلوب');
        if (!row.email?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email)) errors.push('email غير صالح');
        ...
        if (!row.password?.trim()) errors.push('password مطلوب');
        if (!row.governorate_code?.trim()) errors.push('governorate_code مطلوب');
        if (!row.service_slug?.trim()) errors.push('service_slug مطلوب');
        break;
      case 'products':
        if (!row.provider_email?.trim()) errors.push('provider_email مطلوب');
        if (!row.name_ar?.trim()) errors.push('name_ar مطلوب');
        if (!row.category_name_ar?.trim()) errors.push('category_name_ar مطلوب');
        const price = Number(row.price);
        if (isNaN(price) || price < 0) errors.push('price يجب أن يكون رقمًا موجبًا');
        break;
      case 'coupons':
        if (!row.provider_email?.trim()) errors.push('provider_email مطلوب');
        if (!row.code?.trim()) errors.push('code مطلوب');
        if (!['percent', 'fixed'].includes(row.discount_type)) errors.push('discount_type يجب أن يكون percent أو fixed');
        const dv = Number(row.discount_value);
        if (isNaN(dv) || dv <= 0) errors.push('discount_value يجب أن يكون رقمًا موجبًا');
        break;
    }
...
function importAgent(data) {
  ...
  if (!user) {
    ...
    user = { id: run(
      'INSERT INTO users (role, name_ar, name_en, email, phone, password_hash, is_active) VALUES (?,?,?,?,?,?,1)',
      ['agent', name_ar, name_en || null, email.toLowerCase(), phone, passwordHash]
    ).lastId };
  }

  const governorate = get('SELECT id FROM governorates WHERE code = ?', [governorate_code.toUpperCase()]);
  if (!governorate) throw new Error(`محافظة غير موجودة: ${governorate_code}`);

  const existingAgent = get('SELECT id FROM agents WHERE governorate_id = ?', [governorate.id]);
  if (existingAgent) throw new Error(`المحافظة ${governorate_code} لديها وكيل بالفعل`);
  ...
  run(
    ...
    [user.id, governorate.id, Number(commission_rate) || 3, 'active', end.toISOString().replace('T', ' ').slice(0, 19)]
  );
}
...
function importProvider(data, userId) {
  ...
  if (!user) {
    ...
    user = { id: run(
      'INSERT INTO users (role, name_ar, name_en, email, phone, password_hash, is_active) VALUES (?,?,?,?,?,?,1)',
      ['provider', name_ar, name_en || null, email.toLowerCase(), phone, passwordHash]
    ).lastId };
  }
  ...
  if (!service) throw new Error(`خدمة غير موجودة: ${service_slug}`);

  const existingProvider = get('SELECT id FROM providers WHERE user_id = ?', [user.id]);
  if (existingProvider) throw new Error(`المستخدم ${email} لديه مزود بالفعل`);

  run(
    `INSERT INTO providers (user_id, governorate_id, service_id, name_ar, name_en, commission_rate, address, description, website, is_active, is_featured, is_verified)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    [user.id, governorate.id, service.id, name_ar, name_en || null, Number(commission_rate) || 5, address || null, description || null, website || null, Number(is_active) || 1, Number(is_featured) || 0, Number(is_verified) || 0]
  );
}
...
function importProduct(data) {
  ...
  if (!provider) throw new Error(`مزود غير موجود: ${provider_email}`);
  ...
  run(
    `INSERT INTO products (provider_id, category_id, name_ar, name_en, description, price, old_price, stock, images_json, is_active, is_featured)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
    [provider.id, category.id, name_ar, name_en || null, description || null, Number(price), old_price ? Number(old_price) : null, Number(stock) || 0, JSON.stringify(images), Number(is_active) || 1, Number(is_featured) || 0]
  );
}
...
function importCoupon(data) {
  ...
  if (existing) throw new Error(`رمز الكوبون مستخدم: ${code}`);
  ...
  if (discountType === 'percent' && (discountValue < 1 || discountValue > maxPct)) throw new Error(`نسبة الخصم يجب أن تكون بين 1% و ${maxPct}%`);
  if (discountType === 'fixed' && (discountValue < 1 || discountValue > maxFixed)) throw new Error(`الخصم الثابت يجب أن يكون بين 1 و ${maxFixed} دينار`);
  ...
  run(
    `INSERT INTO coupons (code, title, discount_type, discount_value, min_amount, provider_id, starts_at, ends_at, max_uses, per_customer_limit, is_active)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
    [code.toUpperCase(), title || null, discountType, discountValue, Number(min_amount) || 0, provider.id, starts_at || null, ends_at || null, Number(max_uses) || 0, Number(per_customer_limit) || 1, Number(is_active) || 1]
  );
}
  ...
  switch (entity) {
    case 'agents':
      headers = ['id', 'name_ar', 'name_en', 'email', 'phone', 'governorate', 'commission_rate', 'lease_status', 'lease_expires_at', 'created_at'];
      {
        const agentWhere = [];
        const agentParams = [];
        if (status) { agentWhere.push('a.lease_status = ?'); agentParams.push(status); }
    ...
    case 'providers':
      headers = ['id', 'name_ar', 'name_en', 'email', 'phone', 'governorate', 'service', 'commission_rate', 'is_active', 'is_verified', 'is_featured', 'created_at'];
      {
        const provWhere = [];
        const provParams = [];
        if (status) { provWhere.push('p.is_active = ?'); provParams.push(Number(status)); }
...
module.exports = {
  TEMPLATES, getTemplate, parseCSV, validateRows, importRows,
  importAgent, importProvider, importProduct, importCoupon, exportEntity,
};
