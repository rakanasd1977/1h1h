...
function putSettings(body, actor) {
  const updated = [];
  for (const [key, val] of Object.entries((body || {}) as { [k: string]: any })) {
    if (typeof val !== 'object' || val === null || !('value' in val)) {
      throw new ApiError(400, `بنية غير صالحة للمفتاح ${key}`);
    }
    // الأسرار لا تُكشف في الاستجابة ولا تُمسح بفراغ: قيمة فارغة تعني الإبقاء على السر الحالي
    if (isSecret(key) && (val.value === '' || val.value == null)) {
      updated.push(key);
      continue;
    }
...
const VALIDATORS = {
  ...
  smtp_port: (v) => { const n = Number(v); if (isNaN(n) || n < 1 || n > 65535) throw new ApiError(400, 'منفذ SMTP بين 1 و 65535'); },
  require_agent_lease: (v) => { if (v !== '0' && v !== '1' && v !== 'true' && v !== 'false') throw new ApiError(400, 'القيمة يجب أن تكون 0 أو 1'); },
  require_provider_verification: (v) => { if (v !== '0' && v !== '1' && v !== 'true' && v !== 'false') throw new ApiError(400, 'القيمة يجب أن تكون 0 أو 1'); },
  sms_base_url: (v) => { if (v && !/^https?:\/\//i.test(String(v))) throw new ApiError(400, 'رابط بوابة SMS يجب أن يبدأ بـ http:// أو https://'); },
  payment_gateway_mode: (v) => { if (v !== 'sandbox' && v !== 'live') throw new ApiError(400, 'وضع البوابات يجب أن يكون sandbox أو live'); },
};

function putSetting(key, value, label, actor) {
  if (value === undefined) throw new ApiError(400, 'القيمة مطلوبة');
  // سر بلا قيمة = الإبقاء على السر الحالي دون كشفه
  if (isSecret(key) && (value === '' || value == null)) {
    const row = get('SELECT key, value, label FROM settings WHERE key = ?', [key]);
    if (!row) throw new ApiError(404, 'الإعداد غير موجود');
    return { [row.key]: { value: '', label: row.label, sealed: true } };
  }
...
const SECRET_KEYS = new Set(['smtp_pass', 'sms_api_key', 'sms_api_secret']);

function present(row) {
  const secret = isSecret(row.key);
  return { [row.key]: { value: maskSecret(row.key, row.value), label: row.label, ...(secret ? { sealed: true } : {}) } };
}
