...
const { creditWallet, walletTx } = require('../utils/wallet');
...
function approveRecharge(actor, id) {
  ...
  const balanceAfter = transaction(() => {
    ...
    const bal = creditWallet(r.provider_id, Number(r.amount));
    walletTx(r.provider_id, 'recharge', {
      amount: round2(r.amount),
      balanceAfter: bal,
      note: `قبول طلب الشحن ${r.reference} (${paymentLabel(r.payment_method)})`,
