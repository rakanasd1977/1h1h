...
const { deleteRemovedImages, uploadFilePath } = require('../utils/uploads');
const { saveProviderDoc, deleteRemovedProviderDocs } = require('../utils/docStorage');
const { notifyRole } = require('../utils/push');
const { redactOrderForViewer } = require('./orders');
...
function getDashboard(provider) {
  ...
  const type_alerts: Record<string, number> = {};
...
function updateProfile(provider, actor, body) {
  ...
  // تغيير البريد الإلكتروني يتطلب تحققاً منفصلاً (كلمة المرور + تأكيد البريد الجديد) عبر /auth/change-email
  if (email !== undefined && email !== '' && String(email).toLowerCase() !== (userRow.email || '').toLowerCase()) {
    throw new ApiError(400, 'تغيير البريد الإلكتروني يتطلب تحققاً منفصلاً — استخدم مسار تغيير البريد الآمن في إعدادات الحساب');
  }
  ...
  transaction(() => {
    run(
      ...
      [
        ...
        email !== undefined && email !== '' ? String(email).toLowerCase() : userRow.email,
...
function listBookings(provider, query) {
  ...
  const actor = { role: 'provider', provider_id: pid };
  return { rows: rows.map((o) => redactOrderForViewer(actor, o)), meta: { total, page: pg.page, limit: pg.limit, pages: Math.max(1, Math.ceil(total / pg.limit)) } };
}
...
function listRatings(provider, query) {
  ...
  const noReplyCount = get(
    `SELECT COUNT(*) AS c FROM provider_ratings WHERE provider_id = ? AND (reply IS NULL OR reply = '')`,
    [pid]
  ).c;
...
// تحويل صورة مستند مرسلة إلى مرجع مركزي مضغوط في data/docs/<providerId>/.
// يقبل: data:image (يُضغط), /docs/ (موجود مركزي — يُمرَّر), /uploads/ (يُرحَّل ويُضغط),
// وروابط خارجية https (يُمرَّر كما هي). يُعيد { ref, legacy } حيث legacy ملفات /uploads
// القديمة المرحّلة ليُحذفها المنادي بعد نجاح الحفظ.
async function resolveDocSubmission(provider, value, label) {
  if (!value) return { ref: null, legacy: [] };
  const s = String(value);
  if (s.startsWith('data:')) {
    const { url } = await saveProviderDoc(provider.id, s);
    return { ref: url, legacy: [] };
  }
  if (s.startsWith('/docs/')) return { ref: s, legacy: [] };
  if (s.startsWith('/uploads/')) {
    const abs = uploadFilePath(s);
    if (!abs || !fs.existsSync(abs)) throw new ApiError(400, `${label} — الملف المرفوع غير موجود، أعد رفعه`);
    const ext = path.extname(abs).toLowerCase().slice(1);
    const mime = ext === 'png' ? 'image/png' : ext === 'webp' ? 'image/webp' : ext === 'gif' ? 'image/gif' : 'image/jpeg';
    const { url } = await saveProviderDoc(provider.id, `data:${mime};base64,${fs.readFileSync(abs).toString('base64')}`);
    return { ref: url, legacy: [abs] };
  }
  if (s.startsWith('https://') || s.startsWith('http://localhost')) {
    return { ref: s, legacy: [] };
  }
...
async function submitVerification(provider, actor, body) {
  const p = provider;
  ...
  const national = await resolveDocSubmission(p, national_id_image, 'البطاقة الوطنية');
  const residency = await resolveDocSubmission(p, residency_doc_image, 'تأييد السكن');
  const newNational = national.ref ?? p.national_id_image;
  const newResidency = residency.ref ?? p.residency_doc_image;
  run(
  ...
  // مستندات المستودع المركزي القديمة المستبدلة تُحذف؛ وروابط /uploads القديمة أيضاً
  deleteRemovedProviderDocs([p.national_id_image, p.residency_doc_image].join(','), [newNational, newResidency].join(','));
  deleteRemovedImages([p.national_id_image, p.residency_doc_image].join(','), [newNational, newResidency].join(','));
  // ملفات /uploads المؤقتة التي رُحِّلت للمستودع المركزي تُحذف
  for (const abs of [...national.legacy, ...residency.legacy]) {
...
// ===================== الإلزام بالتوثيق (تهيئة مؤقتة: رسائل تذكير فقط) =====================
// عند تفعيل إعداد require_provider_verification يمنع المزود غير الموثق من النشر.
function assertProviderCanPublish(provider) {
  if (Number(settingValue('require_provider_verification', 0)) !== 1) return;
  if (!provider || provider.verification_status === 'approved') return;
  throw new ApiError(403, 'حسابك غير موثق — أرسل البطاقة الوطنية وتأييد السكن من صفحة "ملفي الشخصي" أولاً، ثم يصادق وكيل محافظتك على توثيقك');
}
...
function couponStatus(row) {
  ...
  if (row.ends_at && new Date(String(row.ends_at).replace(' ', 'T') + 'Z').getTime() < Date.now()) return 'expired';
  if (Number(row.max_uses) > 0 && Number(row.used_count) >= Number(row.max_uses)) return 'exhausted';
  return 'active';
}
...
function listCoupons(provider, query) {
  ...
  ).map((r) => ({ ...r, status: couponStatus(r) }));
  const allRows = all(COUPON_SELECT + ' WHERE c.provider_id = ?', [provider.id]);
  const active_count = allRows.filter((r) => couponStatus(r) === 'active').length;
  const used_total = allRows.reduce((s, r) => s + (Number(r.used_count) || 0), 0);
  return {
    rows,
    meta: {
      total,
...
module.exports = {
  ...
  getVerification, submitVerification, assertProviderCanPublish,
