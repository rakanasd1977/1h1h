...
function normalize(body: any): Record<string, any> {
...
function createCoupon(body, actor) {
  ...
  // التحقق من سقف النسبة على النوع النهائي (percent/100) — لا يعتمد على وجود
  // discount_type في نفس الطلب (كان بالإمكان تجاوز السقف بإغفاله ثم افتراضي percent).
  if (n.discount_type === 'percent' && Number(n.discount_value) > 100) {
    throw new ApiError(400, 'نسبة الخصم حتى 100%');
  }
...
function updateCoupon(id, body, actor) {
  ...
  if (n.discount_type === undefined) n.discount_type = row.discount_type;
  if (n.discount_type === 'percent' && Number(n.discount_value !== undefined ? n.discount_value : row.discount_value) > 100) {
    throw new ApiError(400, 'نسبة الخصم حتى 100%');
  }
