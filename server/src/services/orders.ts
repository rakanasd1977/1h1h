...
const { ApiError, toId, round2, paginate, isUniqueViolation, localDayUtcBoundary, assertAmount, assertQuantity, settingValue, isAgentLeaseActive } = require('../utils/helpers');
...
const { walletTx } = require('../utils/wallet');
...
const ORDER_SELECT = `
  SELECT o.*, p.name_ar AS provider_name, p.logo AS provider_logo, p.phone AS provider_phone,
...
// بيانات الزبون الحساسة تُخفى عن المزوّد حتى يقبل الطلب (confirmed)، حمايةً من
// التواصل/الغش خارج المنصة قبل التأكيد. الزبون وحده يرى رقم المزوّد بعد قبول الطلب.
function redactOrderForViewer(actor, o) {
  const out = { ...o };
  if (actor && actor.role === 'provider') {
    const shown = o.status !== 'pending';
    if (!shown) {
      out.customer_name = null;
      out.customer_name_ref = null;
      out.customer_phone = null;
      out.customer_address = null;
    }
  }
  if (actor && actor.role === 'customer') {
    const contactShown = ['confirmed', 'in_progress', 'completed'].includes(o.status);
    if (!contactShown) out.provider_phone = null;
  }
  return out;
}

function decorateOrders(rows, actor = null) {
  ...
  return rows.map((o) => redactOrderForViewer(actor, decorateOrder(o, map)));
}
...
function computeCommissions(provider, governorateId, total, serviceCommissionRate) {
  ...
  let agentAmount = 0;
  if (agent && isAgentLeaseActive(agent)) {
    // حصة الوكيل = نسبة من قيمة الطلب (مستقلة عن حصة المنصة)، مقتصّ ألا تتجاوز إجمالي
    // العمولة لضمان عدم صيرورة حصة المنصة سالبة،
...
function listOrders(actor, query) {
  ...
  if (pg.enabled) {
    ...
    return {
      rows: decorateOrders(rows, actor),
  ...
  return { rows: decorateOrders(rows, actor), meta: null };
}
...
function getOrderById(actor, rawId) {
  ...
  return redactOrderForViewer(actor, decorateOrder(order));
}
  ...
  try {
    ...
    if (idemKey) {
      ...
      if (prior) {
        ...
        if (existingOrder && inOwnershipScope(actor, existingOrder)) return { reused: true, result: redactOrderForViewer(actor, decorateOrder(existingOrder)) };
    ...
    const { orderId, orderNumber } = transaction(() => {
      ...
      for (const [key, qty] of Object.entries(usage)) {
        const [kind, idStr] = key.split(':');
        const id = Number(idStr);
        // الخصم شرطي (قيد المخزون): إن نقص المخزون بعد الفحص الخارجي (سباق متعدد العمليات)
        // يُرفض الطلب وتُتراجع المعاملة بدل بيع أكثر من المتاح.
        if (kind === 'products') {
          const r = run('UPDATE products SET stock = stock - ?, updated_at = datetime(\'now\') WHERE id = ? AND stock >= ?', [qty, id, qty]);
          if (!r.changes) throw new ApiError(409, 'الكمية المطلوبة لم تعد متوفرة، يرجى تحديث السلة وإعادة المحاولة');
        } else if (kind === 'flights') {
          const r = run('UPDATE flights SET seats = seats - ?, updated_at = datetime(\'now\') WHERE id = ? AND seats >= ?', [qty, id, qty]);
          if (!r.changes) throw new ApiError(409, 'المقاعد المطلوبة لم تعد متوفرة، يرجى تحديث السلة وإعادة المحاولة');
        }
      }
      ...
      if (coupon && couponDiscount) {
        // إعادة فحص حدود الاستخدام داخل المعاملة (مصدر حقيقة ذرّي): العدّ خارج المعاملة
        // تقريبي فقط، وسباق المزامنة (عنقود/عدة عمليات) قد يتجاوز max_uses قبل الإدراج.
...
function updateOrderStatus(actor, rawId, body) {
  ...
  transaction(() => {
    ...
    if (status === 'confirmed') {
      ...
      const acceptedCount = Number(get(
        "SELECT COUNT(*) c FROM orders WHERE provider_id = ? AND id != ? AND status IN ('confirmed','in_progress','completed')",
        [order.provider_id, order.id]
      ).c) || 0;
      const isFreeOrder = freeLimit > 0 && acceptedCount < freeLimit;

      if (isFreeOrder) {
        // عرض ترحيبي: أول طلبات المزود الجديد مجاناً بدون عمولة (للمنصة ولا للوكيل)
        commission = 0;
        const total = round2(Number(order.total_amount) || 0);
        run(
          'UPDATE orders SET commission_amount = 0, agent_amount = 0, platform_amount = 0, provider_amount = ? WHERE id = ?',
          [total, order.id]
        );
        const wb = get('SELECT balance FROM provider_wallets WHERE provider_id = ?', [order.provider_id]);
        walletTx(order.provider_id, 'commission', {
          amount: 0,
          balanceAfter: Number(wb ? wb.balance : 0),
          orderId: order.id,
          orderNumber: order.order_number,
          note: `طلب مجاني ضمن العرض الترحيبي (${acceptedCount + 1}/${freeLimit}) — لا عمولة للمنصة ولا للوكيل`,
        ...
        if (commission > 0) {
          ...
          walletTx(order.provider_id, 'commission', {
            amount: -commission,
            agentAmount: Number(order.agent_amount) || 0,
            platformAmount: Number(order.platform_amount) || 0,
            balanceAfter,
            orderId: order.id,
            orderNumber: order.order_number,
            note: `استقطاع عمولة الطلب ${order.order_number}: حصة الوكيل ${round2(order.agent_amount)} + حصة المنصة ${round2(order.platform_amount)}`,
            createdBy: actor.name_ar || actor.role,
          });
        }
      }
    ...
    if (status === 'cancelled' && (order.accepted_at || order.status !== 'pending')) {
      ...
      if (ded) {
        ...
        walletTx(order.provider_id, 'refund', {
          amount: refund,
          agentAmount: Number(ded.agent_amount) || 0,
          platformAmount: Number(ded.platform_amount) || 0,
          balanceAfter: balanceAfterR,
          orderId: order.id,
          orderNumber: order.order_number,
          note: `رد عمولة الطلب الملغي ${order.order_number}`,
          createdBy: actor.name_ar || actor.role,
        });
      }
    }
  ...
  return redactOrderForViewer(actor, decorateOrder(get(ORDER_SELECT + ' WHERE o.id = ?', [id])));
}
