...
function publicProvider(p, locale = 'ar') {
  return {
    ...
    address: p.address,
    // رقم الهاتف لا يُكشف للجمهور — يتاح للزبون داخل تفاصيل طلبه بعد قبول الطلب فقط.
    website: p.website,
    rating: p.rating,
...
function getTopSelling(query, locale = 'ar') {
  ...
  const soldMap: Map<string, number> = buildSoldMap();
  const entries: Array<{ kind: string; item_id: number; sold: number }> = Array.from(soldMap.entries())
    .map(([key, sold]) => {
  ...
  const byKind: Record<string, Array<{ kind: string; item_id: number; sold: number }>> = {};
...
function getHomeLayout() {
  ...
  if (stored === null) return base.sort((a, b) => a.order - b.order);

  const defaultsByKey = new Map(base.map((s) => [s.key, s]));
  const result = [];
  for (const s of stored) {
    if (!s || typeof s.key !== 'string' || !s.key) continue;
    if (defaultsByKey.has(s.key)) {
      const t = { ...defaultsByKey.get(s.key) };
      t.enabled = s.enabled !== false;
      if (typeof s.order === 'number') t.order = s.order;
      if (typeof s.label === 'string' && s.label.trim()) t.label = s.label.trim();
      result.push(t);
    } else {
      result.push({
        key: s.key,
        label: typeof s.label === 'string' && s.label.trim() ? s.label.trim() : 'قسم مخصّص',
        enabled: s.enabled !== false,
        order: typeof s.order === 'number' ? s.order : result.length + 1,
        custom: true,
      });
    }
  }
...
const DEFAULT_PRIVACY_POLICY = `سياسة الخصوصية — سوق الرافدين

نحن في "سوق الرافدين" نولي خصوصيتك اهتماماً بالغاً. توضّح هذه السياسة كيف نجمع ونستخدم ونحمي بياناتك.

١) البيانات التي نجمعها
- بيانات التسجيل: الاسم، البريد الإلكتروني، رقم الهاتف، والعنوان.
- بيانات الحساب والمحفظة وعمليات الطلب والدفع.
- وثائق التحقق للمزودين والوكلاء (هوية، إقامة، إثبات شحن الرصيد) وتُعرض فقط للجهات المخوّلة وليست علنية.
- بيانات الاستخدام التقنية (مثل عنوان IP ونوع المتصفح) لأغراض الأمان والتحليل.

٢) كيف نستخدم بياناتك
- تقديم الخدمات وتنفيذ الطلبات والتواصل بشأنها.
- التحقق من الهوية ومنع الاحتيال وحماية المنصة.
- تحسين تجربة المستخدم وإرسال الإشعارات التي وافقت عليها فقط.

٣) حماية البيانات
نستخدم وسائل حماية تقنية وإدارية (تشفير كلمات المرور، عزل الوثائق الحسّاسة، وصول مصرّح به) لمنع الوصول غير المصرّح به.

٤) مشاركة البيانات
لا نبيع بياناتك. نشارك الحد الأدنى الضروري مع المزود لتنفيذ طلبك، ومع الجهات النظامية عند الإلزام القانوني.

٥) حقوقك
يحق لك الوصول إلى بياناتك وتصحيحها، وحذب حسابك بطلب من الدعم. للاستفسار: support@rafidain.iq`;

const DEFAULT_FAQ = [
  { question: 'ما هي منصة سوق الرافدين؟', answer: 'سوق إلكتروني عراقي يربط الزبون بمزوّدي الخدمة عبر 18 محافظة (متاجر، مطاعم، فنادق، حجز طيران، ومكاتب سفر).' },
  { question: 'كيف أطلب خدمة أو منتجاً؟', answer: 'اختر المزود من الصفحة الرئيسية أو البحث، أضف المنتجات إلى السلة، ثم أكمل الدفع من صفحة الدفع.' },
  { question: 'ما هي طرق الدفع المتاحة؟', answer: 'الدفع عند الاستلام، والدفع الإلكتروني عبر بطاقات الدفع، وشحن رصيد المحفظة حسب توفّرها.' },
  { question: 'كيف أتتبع طلبي؟', answer: 'من قسم "طلباتي" في حسابك يمكنك متابعة حالة الطلب وتفاصيله لحظياً.' },
  { question: 'كيف أصبح مزوّد خدمة أو وكيل محافظة؟', answer: 'سجّل كمزوّد من لوحة المزودين، أو تواصل مع الإدارة للتوكيل كوكيل محافظة حسب الشروط.' },
];

// محتوى قابل للتحرير عبر الإعدادات (مفاتيح: privacy_policy، faq)
function getContent() {
  const pp = get("SELECT value FROM settings WHERE key = 'privacy_policy'");
  const faqRow = get("SELECT value FROM settings WHERE key = 'faq'");
  let faq = DEFAULT_FAQ;
  if (faqRow && faqRow.value) {
    try { faq = JSON.parse(faqRow.value); } catch (e) { faq = DEFAULT_FAQ; }
  }
  return {
    privacy_policy: pp ? pp.value : DEFAULT_PRIVACY_POLICY,
    faq,
  };
}
...
module.exports = {
  ...
  getItemReviews, getDeals, getTopSelling, getGovernorateByGeo, getHomeLayout, getContent,
};
