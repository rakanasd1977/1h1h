...
const { ensureWallet, debitWallet, walletTx } = require('../utils/wallet');
const { assertProviderCanPublish } = require('./provider');
...
function createPromotion({ provider, def, item, durationDays, note = '', user, targetType = 'governorate', governorateIds = [], billing = 'wallet', placement = 'home_top' }) {
  ...
  if (billing === 'wallet') {
    ...
    promoId = transaction(() => {
      const balanceAfter = debitWallet(provider.id, cost);
      walletTx(provider.id, 'promotion', {
        amount: -cost,
        platformAmount: cost,
        balanceAfter,
        note: `ترويج: ${item[def.titleCol]}`,
        createdBy: note,
      });
...
function createProviderPromotion(user, body) {
  ...
  assertProviderCanPublish(provider);
  ...
  if (!isFree) {
    ...
    transaction(() => {
      ...
      walletTx(provider.id, 'promotion', {
        ...
        platformAmount: cost,
        balanceAfter,
        note: `تمديد ترويج: ${row.item_title}`,
        createdBy: user.name_ar || user.email,
      });
