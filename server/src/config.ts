...
// لا نحمّل .env في الاختبارات: كل ملف اختبار يضبط قيمه الخاصة بنفسه،
// و.env ملف تشغيلي (تطوير/إنتاج) وليس من شأنه التأثير على الاختبارات.
...
const config = {
  ...
  // في بيئة التطوير معطَّل افتراضياً لئلا يعرقل التجربة (5 محاولات = قفل 15 دقيقة)،
  // ومفعَّل في الإنتاج والاختبار. يمكن الضبط يدوياً عبر LOCKOUT_ENABLED=1/0.
  lockout: {
    enabled: process.env.LOCKOUT_ENABLED !== undefined
      ? process.env.LOCKOUT_ENABLED === '1'
      : env !== 'development',
  ...
  // ربط الجلسات ببصمة الجهاز لحسابات الامتياز (admin/agent/provider).
  // معطَّل افتراضياً في التطوير لئلا يُطرد مستخدم اللوحات عند تغيّر IP/متصفح.
  // يُفعَّل افتراضياً في الإنتاج والاختبار عبر FINGERPRINT_ENABLED=1.
  fingerprint: {
    enabled: process.env.FINGERPRINT_ENABLED !== undefined
      ? process.env.FINGERPRINT_ENABLED === '1'
      : env !== 'development',
  },
