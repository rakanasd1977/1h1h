...
if (cluster.isPrimary) {
  ...
  const { startCloudBackupScheduler } = require('./services/cloud-backup');
  ...
  // بذر تلقائي في وضع غير الإنتاج (أو عند تفعيل SEED_ALLOW=1) فقط عندما تكون القاعدة فارغة
  // يمنع شاشات الخطأ (مثل "تعذر تحميل المحافظات") الناتجة عن غياب بيانات التجربة بعد حذف server/data
  try {
    const { get: _dbGet } = require('./db/index');
    const dbEmpty = !_dbGet('SELECT 1 FROM users LIMIT 1');
    if (dbEmpty && (process.env.NODE_ENV !== 'production' || process.env.SEED_ALLOW === '1')) {
      console.log('[startup] تشغيل البذر التلقائي (development)…');
      require('./db/seed');
    }
  } catch (se) {
    console.error('[startup] تعذّر تشغيل البذر التلقائي:', se);
  }
