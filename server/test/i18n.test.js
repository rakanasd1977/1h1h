...
test('shared/i18n: t() يترجم العناوين وفق اللغة الحالية', () => {
  const { t, setLocale } = require('../../shared/src/i18n');
  setLocale('ar');
  assert.equal(t('nav.home'), 'الرئيسية');
  setLocale('en');
  assert.equal(t('nav.home'), 'Home');
  assert.equal(t('common.searchPlaceholder'), 'Search for a provider, service or product...');
});

test('shared/i18n: مفتاح غير موجود يُرجَع كما هو + pickLocale يختار الاسم المناسب', () => {
  const { t, pickLocale } = require('../../shared/src/i18n');
  assert.equal(t('missing.key.xyz'), 'missing.key.xyz');
  assert.equal(pickLocale('القاهرة', 'Cairo', 'en'), 'Cairo');
  assert.equal(pickLocale('بغداد', 'Baghdad', 'ar'), 'بغداد');
  assert.equal(pickLocale('البصرة', null, 'en'), 'البصرة');
});

test('shared/i18n: t() يدعم استبدال المتغيرات {x} في النص', () => {
  const { t, setLocale } = require('../../shared/src/i18n');
  setLocale('ar');
  assert.equal(t('catalog.count', { count: 12 }), '12 منتج');
  setLocale('en');
  assert.equal(t('catalog.count', { count: 12 }), '12 products');
  assert.equal(t('profile.member', { name: 'Bronze' }), 'Bronze member');
});
