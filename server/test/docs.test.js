...
async function adminToken() {
  ...
  _adminToken = login.json.data.token;
  return _adminToken;
}
...
// ------------------------- اختبارات التكامل -------------------------
test('GET /api/docs/openapi.json بمصادقة مسؤول → 200 JSON صالب', async () => {
  const r = await api('GET', '/api/docs/openapi.json', { token: await adminToken() });
  assert.equal(r.status, 200, 'يجب أن يكون الوصول لمسؤول فقط');
...
test('GET /api/docs/ بمصادقة مسؤول → 200 HTML فيه Swagger UI', async () => {
  const r = await api('GET', '/api/docs/', { token: await adminToken() });
