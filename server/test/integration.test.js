...
test('إشعارات Web Push: مفتاح VAPID + اشتراك + إلغاء + عدم كسر الأحداث المرتبطة', async () => {
  ...
  const ep = `https://push.example.com/push/${Date.now()}`;
...
test('توثيق المزود بالمستندات: رفع، مراجعة قبول/رفض، وإلغاء', async () => {
  ...
  assert.ok(after.json.data.national_id_image.startsWith(`/docs/${store.id}/`), 'يُخزَّن المستند مركزياً في مجلد المزوّد (وليس data URI ولا /uploads)');
...
test('رفع الصور كملفات: نقطة /upload (مصادقة + قيود)، وقبول روابط /uploads في الشحن والتوثيق', async () => {
  ...
  // رفع مستندات التوثيق برابط /uploads → يُهاجَر ضغطاً إلى /docs/<providerId>/ ويُحذف الأصلي
  ...
  assert.match(stored.national_id_image, new RegExp(`^/docs/${store.id}/`));
  assert.match(stored.residency_doc_image, new RegExp(`^/docs/${store.id}/`));
  assert.ok(!fs.existsSync(filePath), 'الملف الأصلي في /uploads يُحذف بعد الهجرة (لم يعد مصدراً عاماً)');
...
test('حذف الصور: استبدال مستندات التوثيق يهاجر /uploads إلى /docs ويرفع الأصلي ويحذف القديم المركزي', async () => {
  const provToken = await login('provider.demo@rafidain.iq', 'Provider@123');
  ...
  const d1Src = path.join(__dirname, '../data/uploads', d1.replace('/uploads/', ''));
  const d2Src = path.join(__dirname, '../data/uploads', d2.replace('/uploads/', ''));
  assert.ok(fs.existsSync(d1Src), 'المصدر الأول موجود قبل التقديم');

  await api('PUT', '/api/provider/verification', { token: provToken, body: { national_id_image: d1 } });
  const ref1 = get('SELECT national_id_image FROM providers WHERE id = ?', [store.id]).national_id_image;
  assert.match(ref1, new RegExp(`^/docs/${store.id}/`), 'هاجر المرجع إلى /docs/<مزوّد>');
  assert.ok(!fs.existsSync(d1Src), 'الملف الأصلي في /uploads حُذف بعد الهجرة');
  assert.ok(fs.existsSync(path.join(__dirname, '../data/docs', String(store.id), ref1.split('/').pop())), 'النسخة المضغوطة محفوظة في /docs');

  await api('PUT', '/api/provider/verification', { token: provToken, body: { national_id_image: d2 } });
  const ref2 = get('SELECT national_id_image FROM providers WHERE id = ?', [store.id]).national_id_image;
  assert.notEqual(ref1, ref2, 'استُبدل المستند بمرجع جديد');
  assert.ok(!fs.existsSync(path.join(__dirname, '../data/docs', String(store.id), ref1.split('/').pop())), 'المستند القديم في /docs حُذف بعد الاستبدال');
  assert.ok(fs.existsSync(path.join(__dirname, '../data/docs', String(store.id), ref2.split('/').pop())), 'المستند الجديد مركزي ومحفوظ');
  assert.ok(!fs.existsSync(d2Src), 'المصدر الثاني في /uploads حُذف');

  // تنظيف مجلد المستندات المركزي لهذا المزوّد (قاعدة الاختبار مؤقتة)
  fs.rmSync(path.join(__dirname, '../data/docs', String(store.id)), { recursive: true, force: true });
});
...
test('العرض الترحيبي: أول 5 طلبات للمزود الجديد مجاناً (بدون عمولة منصة/وكيل) ثم الإجبار على الشحن', async () => {
  await api('PUT', '/api/settings/provider_free_orders', { token: tokens.admin, body: { value: '5' } });

  const email = `free${Date.now()}@rafidain.iq`;
  const svc = get('SELECT * FROM services ORDER BY id ASC LIMIT 1');
  const gov = get('SELECT * FROM governorates ORDER BY id ASC LIMIT 1');
  const u = run(
    'INSERT INTO users (role, name_ar, email, password_hash, governorate_id, service_type, is_active) VALUES (?,?,?,?,?,?,1)',
    ['provider', 'مزود مجاني', email, await hashPassword('Free@123'), gov.id, svc.slug]
  );
  const pid = run(
    'INSERT INTO providers (user_id, governorate_id, service_id, name_ar, commission_rate, is_active, is_verified) VALUES (?,?,?,?,?,1,1)',
    [u.lastId, gov.id, svc.id, 'مزود مجاني', 5]
  ).lastId;
  const tok = await login(email, 'Free@123');

  // أول 5 طلبات مجانية: تُقبَل برصيد صفر وبدون خصم عمولة
  for (let i = 1; i <= 5; i++) {
    const o = await api('POST', '/api/orders', {
      token: tok,
      body: { provider_id: pid, customer_name: 'زبون مجاني', items: [{ title: 'سلعة', quantity: 1, unit_price: 1000 }] },
    });
    assert.equal(o.status, 201, 'إنشاء الطلب');
    const acc = await api('PUT', `/api/orders/${o.json.data.id}/status`, { token: tok, body: { status: 'confirmed' } });
    assert.equal(acc.status, 200, `الطلب المجاني ${i} يُقبَل حتى برصيد صفر`);
    const w = await api('GET', '/api/wallets/provider', { token: tok });
    assert.equal(w.json.data.balance, 0, 'رصيد المحفظة لا يتأثر بالطلب المجاني');
    const ded = w.json.data.transactions.find((t) => t.type === 'commission' && t.order_id === o.json.data.id);
    assert.ok(ded && ded.amount === 0, 'عمولة الطلب المجاني صفر');
    assert.equal(ded.agent_amount, 0, 'حصة الوكيل صفر في الطلب المجاني');
    assert.equal(ded.platform_amount, 0, 'حصة المنصة صفر في الطلب المجاني');
  }

  // الطلب السادس: استُهلكت الحصة → يجب شحن الرصيد (الرصيد صفر يمنع القبول)
  const o6 = await api('POST', '/api/orders', {
    token: tok,
    body: { provider_id: pid, customer_name: 'زبون', items: [{ title: 'سلعة', quantity: 1, unit_price: 1000 }] },
  });
  const acc6 = await api('PUT', `/api/orders/${o6.json.data.id}/status`, { token: tok, body: { status: 'confirmed' } });
  assert.equal(acc6.status, 400, 'بعد استهلاك الحصة يُمنع القبول برصيد صفر');
  assert.match(acc6.json.message, /شحن المحفظة/, 'رسالة إجبار الشحن تظهر');

  // بعد الشحن يُقبَل الطلب
  const ch = await api('POST', `/api/wallets/${pid}/recharge`, { token: tokens.admin, body: { amount: 1000 } });
  assert.equal(ch.status, 200, JSON.stringify(ch.json));
  const acc6b = await api('PUT', `/api/orders/${o6.json.data.id}/status`, { token: tok, body: { status: 'confirmed' } });
  assert.equal(acc6b.status, 200, 'بعد الشحن يُقبَل الطلب');

  await api('PUT', '/api/settings/provider_free_orders', { token: tokens.admin, body: { value: '5' } });
});

test('أسرار الإعدادات (SMTP/SMS) لا تُكشف في الاستجابة، والفراغ يُبقي السر الحالي', async () => {
  const set = await api('PUT', '/api/settings/smtp_pass', { token: tokens.admin, body: { value: 'very-secret' } });
  assert.equal(set.status, 200, JSON.stringify(set.json));
  assert.equal(get("SELECT value FROM settings WHERE key = 'smtp_pass'").value, 'very-secret', 'السر محفوظ في القاعدة');

  const list = await api('GET', '/api/settings', { token: tokens.admin });
  assert.equal(list.status, 200);
  assert.equal(list.json.data.smtp_pass.value, '', 'كلمة مرور SMTP لا تُكشف في القائمة');
  assert.equal(list.json.data.smtp_pass.sealed, true, 'السر يُعلَّم كمغلق (sealed)');

  const one = await api('GET', '/api/settings/smtp_pass', { token: tokens.admin });
  assert.equal(one.status, 200);
  assert.equal(one.json.data.smtp_pass.value, '', 'القيمة المفردة معقّمة أيضاً');

  // إعادة إرسال قيمة فارغة لا تمسح السر المخزّن
  const keep = await api('PUT', '/api/settings/smtp_pass', { token: tokens.admin, body: { value: '' } });
  assert.equal(keep.status, 200);
  assert.equal(get("SELECT value FROM settings WHERE key = 'smtp_pass'").value, 'very-secret', 'الفراغ يُبقي السر الحالي');

  // رابط بوابة SMS ببروتوكول غير http(s) مرفوض
  const badUrl = await api('PUT', '/api/settings/sms_base_url', { token: tokens.admin, body: { value: 'ftp://sms.local' } });
  assert.equal(badUrl.status, 400, 'رابط SMS ببروتوكول غير http(s) مرفوض');
});
