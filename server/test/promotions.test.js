...
test('فلترة الإعلانات حسب الموضع (home_top / most_ordered)', async () => {
  runWallet(store.id, 1000000);
  const top = await api('POST', '/api/promotions/admin/create', {
    token: tokens.admin,
    body: { provider_id: store.id, item_type: 'products', item_id: product.id, duration_days: 7, target: 'all', billing: 'free', placement: 'home_top' },
  });
  assert.equal(top.status, 200, JSON.stringify(top.json));
  const most = await api('POST', '/api/promotions/admin/create', {
    token: tokens.admin,
    body: { provider_id: store.id, item_type: 'products', item_id: product.id, duration_days: 7, target: 'all', billing: 'free', placement: 'most_ordered' },
  });
  assert.equal(most.status, 200, JSON.stringify(most.json));
  assert.equal(top.json.data.placement, 'home_top');
  assert.equal(most.json.data.placement, 'most_ordered');

  const pubTop = await api('GET', '/api/public/promotions?placement=home_top');
  assert.ok(pubTop.json.data.some((x) => x.id === top.json.data.id), 'home_top يظهر في موضع أعلى الصفحة');
  assert.ok(!pubTop.json.data.some((x) => x.id === most.json.data.id), 'most_ordered لا يظهر في أعلى الصفحة');

  const pubMost = await api('GET', '/api/public/promotions?placement=most_ordered');
  assert.ok(pubMost.json.data.some((x) => x.id === most.json.data.id), 'most_ordered يظهر في قسم الأكثر طلباً');
  assert.ok(!pubMost.json.data.some((x) => x.id === top.json.data.id), 'home_top لا يظهر في الأكثر طلباً');

  const pubDefault = await api('GET', '/api/public/promotions');
  assert.ok(pubDefault.json.data.some((x) => x.id === top.json.data.id), 'بدون موضع يرجع أعلى الصفحة افتراضياً');

  const adminMost = await api('GET', '/api/promotions/all?placement=most_ordered', { token: tokens.admin });
  assert.ok(adminMost.json.data.some((x) => x.id === most.json.data.id), 'فلتر المسؤول بالموضع يعمل');
  assert.ok(!adminMost.json.data.some((x) => x.id === top.json.data.id), 'فلتر المسؤول يستبعد الموضع الآخر');
});
